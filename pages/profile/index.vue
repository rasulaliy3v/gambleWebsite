            ...
            <div class="flex flex-col">
              ...
              <div class="flex items-center">
                <p class="flex items-center font-semibold text-md">
                  {{ user.fullname ? user.fullname : 'Setup' }}
                  <button
        ...
        <div class="p-8 bg-white rounded-xl"> 
          <div class="grid grid-cols-1 gap-3 lg:grid-cols-1">
            <div class="space-y-3">
              ...
              <!-- H2 tag end;-->
              <!-- Reference 1:  -->
              <form @submit.prevent="changeReference1()">
                <label>
                  <span class="block mb-2 opacity-80">Your Reference Code</span>
                  <div class="relative">
                    <t-input
                      v-model="user.referenceCode"
                      aria-readonly="true"
                      class="w-full pl-12"
                    ></t-input>
                    <img
              ...
              <!-- FOrm end; -->
              <!-- Reference 2:  -->
              <form @submit.prevent="changeReference2()">
                <label>
                  <span class="block mb-2 opacity-80">Your Friend Reference Code</span>
                  <div class="relative">
                    <t-input
                      v-model="user.otherCode"
                      aria-readonly="true"
                      class="w-full pl-12"
                    ></t-input>
                    <img
...
export default {
  ...
  mounted() {
    ...
    console.log(id);
    this.$axios.post("http://localhost:3334/getReferenceSystem",{
      id: id
    }).then((response) => {
      ...
      if(data.status == "yes"){
        ...
        let otherCodeValue = data.otherCode;
        let referenceCodeValue = data.referenceCode;
        this.user.otherCode = otherCodeValue;
        this.user.referenceCode = referenceCodeValue;
      }
    });
  },
  methods: {
    async changeReference1(){
      alert("Your reference code cannot be changed.");
    },
    async changeReference2(){
      let value = this.user.otherCode;
      let id = this.$auth.user.id;
      this.$axios.post("http://localhost:3334/inviteReference", {
        id: 14, 
        code: "158575"
      }).then((response) => {
        let data = response.data;
